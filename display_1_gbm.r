...
fsample_input <-
  function(fname, n, seed, header=FALSE, maxn=1e20, na.strings=c("NA","NaN", " "), reader = read.csv)
  {
    set.seed(seed)
    ...
    repeat {
      ...
      if ((n_txt <- length(txt)) == 0L || maxn <= total_lines)
        break
    ...
    reader(textConnection(c(hdr, buf)), header=header, na.strings=na.strings)
}
...
###############################################
LOAD_DATA <- FALSE
RELOAD_PATH <- "submissions/submit1"


###############################################
## Load training data and join with feature
###############################################
training <- fsample_input("input/train.csv", 1000, 1, TRUE,1000000)

XtrainClean = cleanInputDataForGBM(training)
XtrainClean = XtrainClean[, c(3:41)]  

#####################
## Test Data load and clean
#####################
test <- read.table(file="input/test.csv",header=TRUE, sep=",", na.strings=c("NA","NaN", " "), nrows=100000)
...
## GBM
#####################
trainCols = c(1:39) # full
Y <- as.factor(training[,2])
X <- XtrainClean
ret = predictGBM(Y, X, XtestClean[,c(2:40)], trainCols)
Yhattrain = exp(ret[[1]])
Yhattest = exp(ret[[2]])
mogbm = ret[[3]]
